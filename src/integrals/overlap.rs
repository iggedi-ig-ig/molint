...
fn hermite_pp([i, j]: [i32; 2], dist: f64, a: f64, b: f64) -> f64 {
    let p = a + b;
    let q = a * b / p;
        ...
        for (j, b) in basis_b.iter().enumerate() {
            ...
            for (coeff_a, exp_a) in a.iter() {
                for (coeff_b, exp_b) in b.iter() {
                    ...
                    sum += coeff_a
                        * coeff_b
                        * hermite_pp([l1, l2], diff.x, exp_a, exp_b)
                        * hermite_pp([m1, m2], diff.y, exp_a, exp_b)
                        * hermite_pp([n1, n2], diff.z, exp_a, exp_b)
                        * (std::f64::consts::PI / (exp_a + exp_b)).powi(3).sqrt()
                }
            }
